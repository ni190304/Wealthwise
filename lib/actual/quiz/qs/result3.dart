...
import 'package:wealthwise/actual/quiz/qs/qsumm2.dart';
import 'package:wealthwise/actual/quiz/qs/qsumm3.dart';
...
class Result3 extends StatefulWidget {
  Result3({super.key, required this.chosen_answers});
...
class _Result3State extends State<Result3> {
  ...
  List<Map<String, Object>> getSummaryData() {
    ...
    for (var i = 0; i < answered_ques3.length; i++) {
      summary.add({
        'question_index': i,
        'question': questions3[i].text,
        'user_answered': answered_ques3[i],
        'correct_answer': correct_answers3[i],
      });
    }
  ...
  void submit_details() async {
    ...
    await FirebaseFirestore.instance
        .collection('QuizScore3')
        .doc(current_email)
        .set({
      'score': '${correctly_answered3.length} / ${answered_ques3.length}'
    });
  }
                    ...
                    Column(
                      children: [
                        Container(
                          height: 85,
                          width: 85,
                          child: correctly_answered3.isEmpty
                              ? Lottie.asset('lib/animations/anim2.json')
                              : correctly_answered3.length != 0 &&
                                      correctly_answered3.length <= 1
                                  ? Lottie.asset('lib/animations/anim4.json')
                                  : correctly_answered3.length > 1 &&
                                          correctly_answered3.length != 3
                                      ? Lottie.asset(
                        ...
                        // ),
                        Text(
                          correctly_answered3.isEmpty
                              ? 'Sorry, Better luck next time'
                              : correctly_answered3.length <= 3 &&
                                      correctly_answered3.isNotEmpty
                                  ? 'Improvement needed'
                                  : correctly_answered3.length > 3 &&
                                          correctly_answered3.length != 6
                                      ? 'Well done'
                                      : 'Congratulations!!!',
                      ...
                      child: Padding(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Text(
                              '${correctly_answered3.length} / ${answered_ques3.length}',

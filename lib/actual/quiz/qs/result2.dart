...
class Result2 extends StatefulWidget {
  Result2({super.key, required this.chosen_answers});
...
class _Result2State extends State<Result2> {
  ...
  List<Map<String, Object>> getSummaryData() {
    ...
    for (var i = 0; i < answered_ques2.length; i++) {
      summary.add({
        'question_index': i,
        'question': questions2[i].text,
        'user_answered': answered_ques2[i],
  ...
  void submit_details() async {
    ...
    await FirebaseFirestore.instance
        .collection('QuizScore2')
        .doc(current_email)
        .set({
      'score': '${correctly_answered2.length} / ${answered_ques2.length}'
    });
  }
                    ...
                    Column(
                      children: [
                        Container(
                          height: 85,
                          width: 85,
                          child: correctly_answered2.isEmpty
                              ? Lottie.asset('lib/animations/anim2.json')
                              : correctly_answered2.length != 0 &&
                                      correctly_answered2.length <= 1
                                  ? Lottie.asset('lib/animations/anim4.json')
                                  : correctly_answered2.length > 1 &&
                                          correctly_answered2.length != 3
                                      ? Lottie.asset(
                        ...
                        // ),
                        Text(
                          correctly_answered2.isEmpty
                              ? 'Sorry, Better luck next time'
                              : correctly_answered2.length <= 3 &&
                                      correctly_answered2.isNotEmpty
                                  ? 'Improvement needed'
                                  : correctly_answered2.length > 3 &&
                                          correctly_answered2.length != 6
                                      ? 'Well done'
                                      : 'Congratulations!!!',
                      ...
                      child: Padding(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Text(
                              '${correctly_answered2.length} / ${answered_ques2.length}',
